...
function MainTweet({ userData }) {
  const {id: tweetId, texto: content, data: date, nome, usuario, isLikedByUser, isRetweetedByUser } = userData;
  const hourFormatted = format(new Date(date), 'KK:mm a')
  const dateFormatted = format(new Date(date), 'LLL d, yyyy')

  console.log(isRetweetedByUser, isLikedByUser)
  ...
  return (
    <TweetContainer>
      ...
      <Footer>
        ...
        <ListActions comentarios={userData.comentarios} likes={userData.likes} isLikedByUser={isLikedByUser} retweets={userData.retweets} isRetweetedByUser={isRetweetedByUser} tweetId={tweetId}/>
      </Footer>
    </TweetContainer>
  );
}
