...
export class TweetsService {
  ...
  async getTweets(userId: number) {
    const tweets = await this.tweetsRepository
      ...
      .leftJoinAndSelect(
        'tweetPaiDoRetweet.comentarios',
        'commentsDoTweetPaiDoRetweet',
      )
      .leftJoinAndSelect(
        'tweetPaiDoRetweet.usuario',
        'usuarioTweetPaiDoRetweet',
      )
      .leftJoinAndSelect(
      ...
      .leftJoinAndSelect(
        ...
        'retweetDoTweetPaiDoRetweet',
      )
      .leftJoinAndSelect(
        'retweetDoTweetPaiDoRetweet.usuario',
        'usuarioDoRetweetDoTweetPaiDoRetweet',
      )
        ...
        try {
          return {
            ...
            tweetPai: tweet.rtwTweet[0]?.tweetPai
              ? tweet.rtwTweet[0].tweetPai.excluido
  ...
  async deleteTweet(id: number, userId: number) {
    ...
    if (tweet.usuario.id !== userId) {
      throw new UnauthorizedException(
        'Você não tem permissão para excluir este tweet',
      );
    }
    ...
    if (
      tweet.rtwTweet &&
      tweet.rtwTweet.length > 0 &&
      tweet.rtwTweet[0].tweetPai
    ) {
      ...
      if (tweetPai) {
        tweetPai.retweets = tweetPai.retweets.filter(
          (retweet) => retweet.retweet.id !== tweet.id,
        );
  ...
  async getTweetDetails(tweetId: number) {
    ...
    const formattedComments = await Promise.all(
      comments.map(async (comment) => {
        ...
        const retweetsCount = await this.getRetweetsCount(
          comment.comentario.id,
        );
  ...
  async postRetweet(
    ...
    const newRetweet = this.retweetsRepository.create({
      tweetPai,
      retweet: newTweet,
    });
